...
Estimativa de origem de disparos subsônicos baseado em Trajetórias de Massas Pontuais Generalizadas a partir de Projéteis Dinamicamente estáveis em toda a trajetória.
...
V é a velocidade do objeto.
	...
	Para seguir com a solução de [II], podemos, de maneira semelhante à resolução do problema sem arrasto, decompor essa força em duas forças componentes sobre as direções perpendiculares x e y:
	, que, em termos de vx e vy, podem ser escritas como:
	pois  e .

	Considerando  e , vamos reescrever o conjunto de equações diferenciais ordinárias acopladas [IV] como:
, com as condições iniciais: 

	Vamos redefinir as variáveis auxiliares:  e substituir em [IV]:
, com condições iniciais [V].
	...
	Esboçando os passos iniciais para melhor entendimento, vamos calcular:

	Onde h é um passo arbitrariamente escolhido e kα é a média ponderada das inclinações das retas tangentes nos pontos n, n+h/2 e n+h, conforme segue:
	...
	O critério de parada do cálculo é que, sendo um δϕ um valor arbitrário aceitável.
...
Trajetória de Massa Pontual
	...
	Adotar um sistema de referencial fixo na superfície terrestre leva ao aparecimento de forças fictícias, dentre elas a força de Coriolis. Vamos introduzir os termos correspondentes ao efeito do vento e efeito Coriolis no sistema já tratado anteriormente:

onde:
, representa o somatório de todas as forças aerodinâmicas;
, a força Peso;
, o vetor aceleração de Coriolis, devido a rotação da terra.
	...
	A adição da aceleração Coriolis será realizada a adição do vetor.





	Para uma massa pontual, o vetor velocidade  na expressão do arrasto precisa ser	substituído por , pois o arrasto aerodinâmico depende da velocidade relativa ao fluxo de ar e não ao solo.
	...
	Com isso, a expresão [I], em um sistema sem o efeito Coriolis, se torna:
, onde k = (1/2)CdρA/m;
	Com o termo:.
	...
	A mudança de base de vN e vE do eixo N-E para os eixos x e z, em termos de AZ0, pode ser definida por:


	O vetor aceleração de Coriolis, sem componentes da velocidade do vento, pode ser escrito como:
, onde L é a latitude do local do disparo (positivo para hemisfério Norte e negativo para Sul e AZ é o azimute do disparo, medido em sentido horário em relação ao Norte.


	Vamos, agora, adicionar as componentes da velocidade do vento em [III]:

	
Unindo o resultado obtido em [II] com a aceleração de Coriolis [IV] obtemos, em notação vetorial:
...
Considerações sobre o método empregado no programa
	...
	O critério de parada fica:
	, onde δγ é um valor arbritariamente pequeno. Que é um critério semelhante a, utilizado na inclinação do cálculo do arrasto.
	...
	Podemos aplicar a matriz inversa de mudança de base para obter os pontos em termos do Norte e Leste, e não do eixo x:
, que pode ser entendida como a mesma mudança de base anterior através de um ângulo -AZ0.
...
Figura 15: Trajetória do disparo.


Figura 16: Vista do disparo a partir deou Azimute 21.11o. É possível notar o desvio ocasionado pelo vento e Coriolis.
